...
#从一个文本文件中读取整数索引，每行一个整数，并将它们存储在一个列表中
def parse_index_file(filename):
    ...
    return index
#从指定的引文数据集中加载节点特征、标签和图的结构信息，并进行必要的处理，确保数据格式适合后续的机器学习任务
...
#从指定的文件路径加载图数据集的特征、标签和邻接矩阵，并提供数据集的详细信息
def load_graph_data(dataset_name, show_details=False):
    ...
    """
    load_path = "dataset/" + "data_zip/" + dataset_name + "/" + dataset_name
...
parser.add_argument('--dataset', type=str, default='uat')  #'BlogCatalog'  'Flickr' 'cora'  'citeseer'  'pubmed'
...
if __name__ == "__main__":
    ...
    elif dataset_str in other_dataset_list:
        attribute_dense, adj_dense, cat_labels = load_graph_data(dataset_str)
